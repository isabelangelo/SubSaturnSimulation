...
/* read in control parameters */
	...
	t_i = 0;
	t_b=t_i-Tprint-1;
	t_inter=fabs(t_f-t_i)/40000000;//4000;//10000;//100.;// /100000;//100000.;
...
//exit(1);
	...
	fprintf(stderr,"	R1=%le R2=%le  [AU] Spin period: %le %le [day] \n"\
		,R1,R2,spin1,spin2);
	...
	//	QoftV1=4.*kL1*4.*M_PI*M_PI*m1*(2./365.)*TV1/(3.*SQR(1.+2.*kL1)*CBE(R1)*2.*M_PI);
	QoftV1=4.*kL1*4.*M_PI*M_PI*m1*(2.)*TV1/(3.*SQR(1.+2.*kL1)*CBE(R1)*2.*M_PI);
	//	QoftV2=4.*kL2*4.*M_PI*M_PI*m2*(2./365.)*TV2/(3.*SQR(1.+2.*kL2)*CBE(R2)*2.*M_PI);
	QoftV2=4.*kL2*4.*M_PI*M_PI*m2*(2.)*TV2/(3.*SQR(1.+2.*kL2)*CBE(R2)*2.*M_PI);
	...
	*/
	...
	//	printf("%lf\t",xp[j]);
	//printf("%1.6lf \t %1.6lf \t %1.6lf \t %1.6lf  %1.6lf \t  %1.6lf \t  %1.6lf   \t  %1.6lf \t  %1.6lf \t  %1.6lf \t  %1.6lf \t  %1.6lf\n"x,e1,e2,g1,g2,a1,i1*180./M_PI,i2*180./M_PI,i*180./M_PI,spin1h,spintot,beta*180./M_PI,vp[1][j]*180/M_PI);
