	...
	void (*derivs)(double, double [], double []))
{
	...
	double eps1,errmax,fact,h,red=0,scale=1.0,work,wrkmin,xest;
	...
	for (;;) {
		for (k=1;k<=kmax;k++) {
			xnew=(*xx)+h;
			if (h==0.) {
			  fprintf(stderr,"step size underflow in bsstep  - assuming that  TF had e->0\n");
			  // k=kmax;
			  break;
			}
			//nrerror("step size underflow in bsstep");			
			//if (xnew == (*xx)) nrerror("step size underflow in bsstep");
			//	fprintf(stderr,"bsstep: before derivs\n");
			mmid(ysav,dydx,nv,*xx,h,nseq[k],yseq,derivs);
			/*	if (flgout==1){
			  //k=kmax;
			  break;
			  }*/
			//			fprintf(stderr,"bsstep after derivs\n");
			xest=SQR(h/nseq[k]);
